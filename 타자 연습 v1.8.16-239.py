...
def setting():#설정
    ...
    try:
        ...
        tkinter.messagebox.showerror(type(excep), f"{excep}\n\n{error_message[type(excep)]}");
        error.append(type(excep));
def enable_topmost():#항상 위에 표시
    global topmost, topmost_temp;
    try:
...
class main():
    def  __init__(self, root):
        #if root == None: root = {};
        global window1, window2, menu, window2_askokcancel, n, source, source2, error_message, topmost, topmost_temp;
        self.root = root;
        try:
                ...
                while menu_suc == 0:
                    ...
                    if dictionary == []:
                        ...
                        if (menu == 1) or (menu == "타자 연습") or (menu == "타자연습"):
                            ...
                            window2.geometry(window2_size);
                            window2.attributes("-topmost", topmost)
                            ...
                            topmost_temp = tkinter.BooleanVar();#문제는 여기
                            if topmost == 1 or True:
                                topmost_temp.set(True);#print("궁'예'지책");
                                print(topmost_temp.get());
                            elif topmost == 0 or False:
                                topmost_temp.set(False);
                                print(topmost_temp.get());
                            
                            tkinter.ttk.Checkbutton(windowm1, text="창 항상 위에 표시", variable = topmost_temp, command = enable_topmost).grid(row = 0, column = 0);
                            tkinter.ttk.Button(windowm1, text = "저장하고 나가기", command = windowm1.destroy).grid(row = 2, column = 0);
                            windowm1.mainloop();
                        ...
                        window1.geometry(window1_size);
                        #print("2");
                        window1.attributes("-topmost", topmost);
                        ...
                        tkinter.ttk.Button(window1, text = "설정", command = setting, width = 30, compound = 'c').grid(row = 1, column = 1, columnspan = 2);
                        tkinter.ttk.Button(window1, text = "종료", command = shutdown, width = 15, compound = 'c').grid(row = 2, column = 1, columnspan = 2);
